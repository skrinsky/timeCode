...
class Trainer:
    ...
    def __init__(
        ...
        # --- Optimizer ---
        params = list(self.synthesizer.parameters()) + list(self.adsr_encoder.parameters())
    ...
    def _train_step(self, batch: dict) -> float:
        ...
        audio_target = batch["audio"].to(self.device)        # [B, n_samples]
        f0_hz        = batch["f0_hz"].to(self.device)        # [B]
        ...
        # Encode ADSR → stage_position, adsr_vector
        ...
        # Compute analytic gate per sample for each clip in batch
        gate = self._compute_batch_gate(
            A, D, S, R, note_dur, n_samples
        )  # [B, n_samples]
